...
const API_URL = `${window.location.protocol}//${window.location.hostname}:8080/api/mercado`;

const GestionMercado = ({ onNavigateToFolder }) => {
  ...
  // --- ESTADO DEL FORMULARIO (SIN ubicación ni garantía) ---
  ...
  const cargarMercado = useCallback(async () => {
    ...
    try {
      ...
      setError(err.response?.status === 401 ? 'Sesión expirada. Vuelve a iniciar sesión.' : 'No se pudo cargar la información de mercado.');
    } finally {
      setCargando(false);
    }
  }, [getAuthHeaders]);
...
const handleSubmit = async (e) => {
  ...
  try {
    if (archivo) {
      // Siempre que haya archivo, se usa FormData
    ...
    toast.success(form.id ? 'Elemento de mercado actualizado' : 'Elemento de mercado registrado exitosamente');
    resetForm();
    cargarMercado();
  } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success('Elemento de mercado eliminado correctamente');
    } catch (err) {
  ...
  const exportarAExcel = () => {
    const datos = productos.map(p => ({
      ...
      // Ya no se exportan Ubicación ni Garantía
    }));
    const hoja = XLSX.utils.json_to_sheet(datos);
    const libro = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(libro, hoja, "Mercado");
    XLSX.writeFile(libro, `MENELEC_Mercado_${new Date().toLocaleDateString()}.xlsx`);
  ...
  if (error) {
    return (
      <div style={styles.errorContainer}>
        ...
        <button onClick={() => { setCargando(true); cargarMercado(); }} style={styles.retryButton}>
  ...
  return (
    <div style={styles.container}>
        ...
        <div style={styles.modalOverlay} onClick={() => setProductoSeleccionado(null)}>
          <div style={styles.fichaTecnica} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.fichaGrid}>
              ...
              <div style={styles.fichaInfoItem}>
                <label style={styles.miniLabelFicha}><FaCalendarAlt /> FECHA COTIZACIÓN</label>
                <p style={styles.pDetail}>{productoSeleccionado.fechaCotizacion || 'N/A'}</p>
              </div>
              <div style={styles.fichaInfoItem}>
                <label style={styles.miniLabelFicha}><FaDollarSign /> PRECIO</label>
                <p style={styles.pDetail}>
                  ${Number(productoSeleccionado.precio).toLocaleString()} COP
                </p>
              </div>
            </div>
      ...
      <header style={styles.header}>
        <div>
          <div style={styles.statusBadge}>MENELEC GESTIÓN DE MERCADO</div>
          <h1 style={styles.title}>Gestión de Mercado</h1>
          <p style={styles.subtitle}>Administración de mercado de forma inteligente</p>
        </div>
        <div style={styles.headerActions}>
          ...
          <button onClick={cargarMercado} style={styles.refreshButton} title="Actualizar">
            <FaSyncAlt />
          </button>
        </div>
      </header>
      ...
      <div style={styles.mainGrid}>
        {/* PANEL IZQUIERDO: FORMULARIO (SIN LOGÍSTICA) */}
        <aside style={styles.sidebar}>
          <section style={styles.glassCard}>
            <h3 style={styles.cardTitle}>{form.id ? 'EDITAR ELEMENTO' : 'NUEVO REGISTRO'}</h3>
            <form onSubmit={handleSubmit} style={styles.form}>
              <div style={styles.fieldGroup}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <input type="text" placeholder="Marca" style={{ ...styles.input, flex: '1 1 150px' }} value={form.marca} onChange={e => setForm({ ...form, marca: e.target.value })} />
                  <input type="text" placeholder="Categoría" style={{ ...styles.input, flex: '1 1 150px' }} value={form.categoria} onChange={e => setForm({ ...form, categoria: e.target.value })} required />
                </div>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <button type="submit" style={styles.btnSave}>
                {form.id ? <><FaSave /> ACTUALIZAR</> : <><FaPlusCircle /> GUARDAR ELEMENTO</>}
              </button>
        ...
        <section style={styles.glassCard}>
          {productosOrdenados.length === 0 ? (
            ...
            <div style={styles.cardGrid}>
              {productosOrdenados.map(p => (
                <div key={p.id} onClick={() => setProductoSeleccionado(p)} style={styles.productCard} onMouseEnter={() => setHoveredRow(p.id)} onMouseLeave={() => setHoveredRow(null)}>
                  ...
                  <div style={styles.cardFooter}>
                    <span style={styles.cardPrecio}>${Number(p.precio).toLocaleString()}</span>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button onClick={(e) => { e.stopPropagation(); setForm({ ...p, precio: p.precio ?? 0, id: p.id }); }} style={styles.btnActionEdit}><FaEdit size={14} /></button>
      ...
      {deleteConfirm.show && (
        <div style={styles.confirmOverlay}>
          <div style={styles.confirmBox}>
            <p style={{ marginBottom: '20px', color: '#f1f5f9', fontSize: '16px' }}>¿Eliminar permanentemente este elemento?</p>
...
const EmptyState = ({ busqueda }) => (
  <div style={{ textAlign: 'center', padding: '60px 20px', color: '#475569', background: 'rgba(15, 23, 42, 0.5)', borderRadius: '24px', border: '2px dashed #1e293b' }}>
    ...
    <p style={{ fontSize: '18px', fontWeight: '600', color: '#94a3b8' }}>{busqueda ? 'Sin resultados' : 'No hay elementos registrados'}</p>
    <p style={{ fontSize: '14px', color: '#64748b' }}>{busqueda ? `No se encontraron elementos que coincidan con "${busqueda}"` : 'Comienza agregando un nuevo elemento desde el formulario'}</p>
  </div>
);
